...
#define EXPECT_EQ_STRING(expect, actual, alength) \
    EXPECT_EQ_BASE(sizeof(expect) - 1 == alength && memcmp(expect, actual, alength) == 0, expect, actual, "%s")/*比较s1 和s2 所指的内存区间前n 个字符,完全相等返回0*/
...
static void test_parse_expect_value() {    //代码重构
...
static void test_parse_invalid_value() {
	...
	TEST_ERROR(LEPT_PARSE_INVALID_VALUE, ".123"); /* at least one digit before '.'小数点前后必须至少有一个数字 */
